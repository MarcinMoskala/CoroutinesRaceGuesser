...
suspend fun generateCollectionProcessingChallenge(level: Level): CollectionProcessingChallenge {
    while (true) {
        val challenge = generateCollectionProcessingChallengeOrNull(level.stepsNum, level.fruitsNum)
        if (challenge != null) return challenge
        yield()
    }
}

fun KType.isSubtypeOf(other: KType) = when (this.classifier) {
    List::class -> other.classifier in listSubclasses &&
            arguments.zip(other.arguments).all { (l, r) -> l.type?.isSubtypeOf(r) == true }

    Set::class -> other.classifier in setSubclasses &&
            arguments.zip(other.arguments).all { (l, r) -> l.type?.isSubtypeOf(r) == true }

    Collection::class -> other.classifier in collectionSubclasses &&
            arguments.zip(other.arguments).all { (l, r) -> l.type?.isSubtypeOf(r) == true }

    Map::class -> other.classifier == Map::class &&
            arguments.zip(other.arguments).all { (l, r) -> l.type?.isSubtypeOf(r) == true }

    else -> this == other
}

fun KType.isSubtypeOf(other: KTypeProjection): Boolean = when (other.variance) {
    null -> true
    KVariance.INVARIANT -> this == other.type
    KVariance.OUT -> this.isSubtypeOf(other.type!!)
    KVariance.IN -> other.type!!.isSubtypeOf(this)
}

private val listSubclasses = setOf(
    Collection::class,
    Iterable::class,
    List::class,
)
private val setSubclasses = setOf(
    Collection::class,
    Iterable::class,
    Set::class,
)
private val collectionSubclasses = setOf(
    Collection::class,
    Iterable::class,
)

suspend fun generateCollectionProcessingChallengeOrNull(
    steps: Int,
    fruitsNum: Int,
): CollectionProcessingChallenge? {
    val start = List(fruitsNum) { allFruits.random() }
...
val processors = listOf(
    ...
    ) { it.filter { it.price > 5.0 } },
    processor<Iterable<Fruit>, List<Fruit>>(
        "filter { it.price >= 10.0 }",
        ProcessorCategory.Filter
    ) { it.filter { it.price >= 10.0 } },
    processor<Iterable<Fruit>, List<Fruit>>(
        "filter { it.price < 15.0 }",
        ProcessorCategory.Filter
    ) { it.filter { it.price < 15.0 } },
    processor<Iterable<Fruit>, List<Fruit>>(
        "filter { it.price <= 7.0 }",
        ProcessorCategory.Filter
    ) { it.filter { it.price <= 7.0 } },
    processor<Iterable<Fruit>, List<String>>(
        "map { it.name }",
        ProcessorCategory.Map
    ) { it.map { it.name } },
    processor<Iterable<Fruit>, List<Color>>(
        "map { it.color }",
        ProcessorCategory.Map
    ) { it.map { it.color } },
    processor<Iterable<Fruit>, List<Double>>(
        "map { it.price }",
        ProcessorCategory.Map
    ) { it.map { it.price } },
    ...
    processor<Iterable<Fruit>, List<Fruit>>(
        "distinct()",
        ProcessorCategory.Distinct
    ) { it.distinct() },
    processor<Iterable<Fruit>, List<Fruit>>(
        "sortedBy { it.price }",
        ProcessorCategory.Sort
    ...
    // List<Color>
    processor<Iterable<Color>, List<String>>(
        "map { it.name }",
        ProcessorCategory.Map
    ) { it.map { it.name } },
    processor<Iterable<Color>, List<Color>>(
        "distinct()",
        ProcessorCategory.Distinct
    ) { it.distinct() },
    ...
    processor<Iterable<Color>, List<Boolean>>(
        "map { it == Color.Red }",
        ProcessorCategory.Map
    ) { it.map { it == Color.Red } },
    processor<Iterable<Color>, List<Boolean>>(
        "map { it == Color.Green }",
        ProcessorCategory.Map
    ) { it.map { it == Color.Green } },

    // List<Double>
    processor<Iterable<Double>, List<Int>>(
        "map { it.toInt() }",
        ProcessorCategory.Map
    ) { it.map { it.toInt() } },
    processor<Iterable<Double>, List<Double>>(
        "map { it.toInt() }",
        ProcessorCategory.Map
    ) { it.filter { it >= 3.0 } },
    processor<Iterable<Double>, List<Double>>("sorted()", ProcessorCategory.Sort) { it.sorted() },
    ...
    processor<Iterable<Boolean>, List<Boolean>>(
        "map { !it }",
        ProcessorCategory.Map
    ) { it.map { !it } },

    // Iterable<Int>
    processor<Iterable<Int>, List<Int>>(
        "filter { it > 6 }",
        ProcessorCategory.Filter
    ) { it.filter { it > 6 } },
    processor<Iterable<Int>, List<Int>>(
    ...
    processor<Iterable<Int>, List<Int>>("sorted()", ProcessorCategory.Sort) { it.sorted() },
    processor<Iterable<Int>, List<Int>>(
        "sortedDescending()",
        ProcessorCategory.Sort
    ) { it.sortedDescending() },
    ...
    processor<Map<Fruit, Color>, _>(
        ...
        ProcessorCategory.Filter
    ) { it.filter { it.value == Color.Red } },
    processor<Map<Fruit, Color>, _>(
        "mapKeys { it.key.color }",
        ProcessorCategory.Filter
    ) { it.mapKeys { it.key.color } },
    processor<Map<Fruit, Color>, _>(
        "map { it.key.color }",
        ProcessorCategory.Filter
    ) { it.map { it.key.color } },
    processor<Map<Fruit, Color>, _>(
        "mapValues { it.key.price }",
        ProcessorCategory.Filter
    ) { it.mapValues { it.key.price } },
    ...
    // List<String>
    processor<Iterable<String>, List<Int>>(
        "map { it.length }",
        ProcessorCategory.Map
    ) { it.map { it.length } },
    processor<Iterable<String>, List<String>>("sorted()", ProcessorCategory.Sort) { it.sorted() },
...
val KType.simpleName: String
    get() = classifierName + if (arguments.isNotEmpty()) "<${arguments.joinToString { it.type?.simpleName.orEmpty() }}>" else ""
val KClassifier.name: String
    get() = (this as? KClass<*>)?.simpleName.orEmpty()
...
val allFruits = listOf(
    Fruit("🍎", "Apple", 2.1, Color.Red),
    Fruit("🍌", "Banana", 4.75, Color.Yellow),
    Fruit("🍇", "Grape", 9.4, Color.Red),
    Fruit("🍊", "Orange", 6.5, Color.Orange),
    Fruit("🍏", "Green Apple", 2.5, Color.Green),
    Fruit("🍆", "Eggplant", 4.3, Color.Purple),
    Fruit("🍉", "Watermelon", 3.5, Color.Red),
    Fruit("🍑", "Peach", 5.1, Color.Orange),
    Fruit("🍓", "Strawberry", 15.0, Color.Red),
    Fruit("🍍", "Pineapple", 7.0, Color.Yellow),
    Fruit("🥝", "Kiwi", 4.1, Color.Green),
    Fruit("🍒", "Cherry", 8.2, Color.Red),
    Fruit("🍋", "Lemon", 7.1, Color.Yellow),
    Fruit("🍈", "Melon", 9.6, Color.Green),
    Fruit("🍐", "Pear", 3.5, Color.Green),
    Fruit("🥑", "Avocado", 22.4, Color.Green),
    Fruit("🍅", "Tomato", 4.3, Color.Red),
    Fruit("🥦", "Broccoli", 10.5, Color.Green),
    Fruit("🌶️", "Pepper", 43.6, Color.Red),
    Fruit("🌽", "Corn", 5.2, Color.Yellow),
    Fruit("🥕", "Carrot", 4.5, Color.Orange),
    Fruit("🍠", "Sweet Potato", 6.5, Color.Orange),
)
...
inline fun <reified T, reified R> processor(
    ...
    from = typeOf<T>(),
    to = typeOf<R>(),
    process = { if (it == null) null else process(it as T) },
    category = category
)
